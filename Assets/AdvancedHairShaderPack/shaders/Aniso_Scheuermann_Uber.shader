...
Shader "Advanced Hair Shader Pack/Aniso Scheuermann Uber" 
{
	Properties 
	{
...
//        _Phong ("Phong Strengh", Range(0,1)) = 0.5
	...
	SubShader 
	{
		//change Transparent to Geometry or AlphaTest to recieve shadows but will not be transparent with Environment!
		//also in Unity5 the extra added alpha causes the alpha blended area to not recieve shadows.
		...
		//tessellate:tessEdge tessphong:_Phong nolightmap
        //#include "Tessellation.cginc"
			...
			{	
				UNITY_INITIALIZE_OUTPUT(Input, o);	
			 	#if ANISO_SKINNED_ON
			 		o.tangent_input = normalize( mul( unity_ObjectToWorld, float4( v.tangent.xyz, 0.0 ) ).xyz );
				#else
...
//	        }
		...
		#pragma surface surf Hair alpha vertex:vert
			...
			sampler2D _MainTex, _SpecularTex, _NormalTex, _CombTex, _AOTex, _ColorMaskTex;
			float _SpecularMultiplier, _SpecularMultiplier2, _PrimaryShift, _SecondaryShift, 
			       _SpecularPower, _BumpPower, _AOPower, _RimStrength, _RimMultiplier, _BlendAlphaCut, _Cutoff;
			fixed4 _SpecularColor, _Color, _SpecularColor2, _ColorR, _ColorG, _ColorB, _ColorA, _RimColor;
